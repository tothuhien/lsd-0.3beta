...
void printHelp( void )
{
    ...
    printf(BOLD"OPTIONS\n"
               ...
               FLAT"\t   Then confidence intervals can be visualised by label `CI_height` using Figtree with node bars function.\n"
               ...
               FLAT"\t   In the partition file, each line contains the name of the part and a list of subtrees whose branches are supposed to have the \n"
               ...
               FLAT"\t   correspond to the tips of the subtree. If the tips of the subtree are not defined (so there's only the defined root), then by \n"
               ...
               FLAT"\t   This option is used to specify the length of the multiple sequences that were used to build the input trees. This parameter is used to \n"
               FLAT"\t   compute variances with option -v and confidence intervals with option -f. By default it is 1000.\n"
